...
rooms = []
while True:
    url = f"https://www.otodom.pl/pl/oferty/wynajem/mieszkanie/wroclaw?distanceRadius=0&page={j}&limit=72&market=ALL&ownerTypeSingleSelect=ALL&locations=%5Bcities_6-39%5D&viewType=listing"
    r = requests.get(url)
    html = r.text
...
df.to_csv("otodom_flats.csv", index=False,sep=";")
df.to_excel("otodom_flats.xlsx", index=False)
